...
for (i in 1:nrow(data_setup)) {
  ...
  modelling_data <- 
    ...
    pins::pin(x = ., 
              name = paste("modelling_data", "_", 
                           data_setup$return_target[i], 
                           "_ret_", data_setup$return_window[i], 
                           "_wind", sep = ""), 
              description = paste("Modelling Data for Trading Strategy", " using ", 
                                  data_setup$return_target[i] * 100 - 1, 
                                  "% return target & ", data_setup$return_window[i], 
                                  " days window", sep = ""), 
              board = board_nm)
