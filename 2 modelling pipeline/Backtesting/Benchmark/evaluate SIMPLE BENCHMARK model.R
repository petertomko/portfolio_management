...
eval_prophet <-
  ...
  dplyr::mutate(
    pred_returns = purrr::map(train_eval_df, function(df){
      
      # df <- modelling_df$test_eval_df[[1]]
...
asset_returns <-
  eval_prophet %>% 
  dplyr::ungroup() %>% 
  dplyr::select(ticker, price_type, created_at, pred_returns) %>% 
  ...
  dplyr::mutate(
    month_w = as.Date(created_at) %m-% months(1)
  ) %>% 
  dplyr::filter(as.Date(created_at) > as.Date(ds) & as.Date(ds) >= as.Date(month_w)) %>% 
  dplyr::select(-month_w) %>% 
  as.data.frame()
...
for (i in unique(asset_returns$created_at)) {
  for (j in unique(asset_returns$price_type)) {
    ...
    cov_mtx <- 
      cov(asset_returns %>% 
            dplyr::filter(created_at == i & price_type == j) %>% 
            dplyr::select(-created_at, -ds, -price_type))
    
    stats_tbl <- 
      asset_returns %>%
      dplyr::filter(created_at == i & price_type == j) %>% 
      dplyr::select(-price_type) %>% 
...
portfolio_returns_df <-
  ...
  modelling_df %>% 
  dplyr::ungroup() %>% 
  dplyr::select(ticker, price_type, created_at, test_df) %>% 
  tidyr::unnest(c(test_df)) %>% 
  dplyr::filter(!(ticker %in% c("DPW", "SOAN"))) %>% 
  as.data.frame() %>% 
  distinct() %>%
  dplyr::select(ticker, price_type, ds, created_at) %>% 
  as.data.frame() %>% 
